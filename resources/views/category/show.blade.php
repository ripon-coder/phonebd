@extends('layouts.app', [
    'title' => $category->meta_title ?? $category->name . ' Mobile Phones in Bangladesh',
...
@section('content')
    ...
    <nav class="flex mb-3 text-sm text-slate-500" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li class="inline-flex items-center">
                <a href="{{ route('categories.index') }}" class="inline-flex items-center hover:text-slate-900 transition-colors">
                    <svg class="w-4 h-4 text-slate-400" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path>
                    </svg>
                    <span class="ml-1 text-sm font-medium text-slate-700 md:ml-2">Categories</span>
                </a>
            </li>
            <li>
                <div class="flex items-center">
                    ...
                    <span class="ml-1 font-medium text-slate-900 md:ml-2">{{ $category->name }}</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="mb-4 mt-2">
        ...
        <div class="bg-white rounded-sm border border-slate-200 p-2 mb-3 flex items-center gap-5">
            ...
            <div>
                <h1 class="text-lg md:text-2xl font-bold text-slate-900 mb-1">{{ $category->name }}</h1>
                <p class="text-sm text-slate-500">{{ $category->meta_description ?? 'Explore ' . $category->name . ' smartphones.' }}</p>
            </div>
        </div>
             ...
             <div class="w-full">
                 <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-3 gap-4">
                    <div class="flex items-baseline justify-between w-full sm:w-auto sm:justify-start gap-2">
                        <h2 class="text-lg md:text-xl font-bold text-slate-900 tracking-tight">{{ $category->name }} Devices</h2>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-1 md:gap-3">
                    ...
                    @empty
                        <div class="col-span-full flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <p class="text-slate-500">We couldn't find any devices in this category matching your filters.</p>
                        </div>
                    @endforelse
                </div>
