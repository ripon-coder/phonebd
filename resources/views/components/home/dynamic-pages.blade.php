...
@if($dynamicPages->isNotEmpty())
    <div class="mb-6">
        <div class="flex items-center justify-between mb-3 px-1">
            <h2 class="text-md lg:text-lg font-semibold text-slate-900 tracking-tight">Buying Guide</h2>
        </div>

        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            @foreach($dynamicPages as $page)
                <a href="{{ route('dynamic_pages.show', $page->slug) }}" 
                   class="group flex items-center justify-between px-3 py-2 bg-white border border-slate-200 rounded-sm hover:border-slate-800 hover:text-slate-900 transition-all duration-200">
