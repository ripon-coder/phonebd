            ...
            <div class="bg-white rounded-sm p-3 md:p-3 shadow-sm border border-slate-100">
                <div class="grid grid-cols-2 gap-3 md:gap-6 items-stretch">
                    ...
                    <div class="relative group h-full" x-data="{ lightboxOpen: false }">
                        <div @click="lightboxOpen = true" class="h-full bg-slate-50 rounded-sm overflow-hidden flex items-center justify-center w-full md:max-w-sm md:mx-auto cursor-zoom-in">
                            @if ($product->image)
                                <img loading="lazy" src="{{ $product->getImageUrl('image') }}" alt="{{ $product->title }}" class="w-full h-full object-contain max-h-[220px] md:max-h-full">
                            @else
                    ...
                    {{-- Info --}}
                    <div>
                        <h1 class="text-xl md:text-2xl font-bold text-slate-900 tracking-tight mb-1">{{ $product->title }}</h1>
                        ...
                        @foreach($similarPriceProducts as $similar)
                            <a href="{{ route('product.show', ['category_slug' => $similar->category->slug, 'product' => $similar->slug]) }}" class="flex items-center gap-4 p-3 hover:bg-slate-50 transition-colors group">
                                <div class="w-20 h-20 bg-slate-50 rounded-sm flex items-center justify-center shrink-0 border border-slate-100">
                                    @if($similar->image)
                                        <img loading="lazy" src="{{ $similar->getImageUrl('image') }}" alt="{{ $similar->title }}" class="w-full h-full object-contain mix-blend-multiply">
                                    @else
